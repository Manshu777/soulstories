...
<div class="chapter-editor-shell min-h-[calc(100vh-4rem)] pb-16" x-data="chapterEditorPageEdit()" x-init="init()">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-4">
        <nav class="text-sm text-slate-500 mb-6 flex flex-wrap items-center gap-2">
            <a href="{{ route('diary.stories.edit', $story) }}" class="hover:text-violet-700 font-medium">{{ $story->title }}</a>
            <span class="text-slate-300">/</span>
            <span class="text-slate-700">{{ $chapter->chapter_title }}</span>
        </nav>

        @if(session('success'))
            <p class="mb-4 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-800 px-4 py-3 text-sm">{{ session('success') }}</p>
        @endif
        @if($errors->any())
            <ul class="mb-4 rounded-2xl bg-red-50 border border-red-100 text-red-700 px-4 py-3 text-sm list-disc list-inside">
        ...
        <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-8">
            <div>
                ...
                <p class="text-slate-500 mt-2 text-sm sm:text-base">
                    Chapter {{ $chapter->chapter_number }} · <span class="text-slate-700 font-medium">{{ $story->title }}</span>
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 xl:gap-8 relative">
            <div class="xl:col-span-8 space-y-4">
                ...
                <div x-show="audioPreview" x-cloak class="rounded-2xl bg-white/80 border border-violet-100 p-4 shadow-sm">
            ...
            <aside class="xl:col-span-4 space-y-5">
                <div class="rounded-[1.75rem] bg-white/90 backdrop-blur-md border border-white/70 shadow-lg shadow-violet-200/40 p-6">
                    ...
                    <input type="number" x-model="continueAfter" min="50" placeholder="e.g. 300"
                        class="w-full rounded-2xl border border-violet-100 px-4 py-3 mb-4 focus:ring-2 focus:ring-violet-200 outline-none">
                    <label class="block text-sm font-medium text-slate-600 mb-1.5">Spotify playlist URL</label>
                    <input type="url" x-model="spotifyPlaylist" placeholder="Optional"
                        class="w-full rounded-2xl border border-violet-100 px-4 py-3 mb-5 focus:ring-2 focus:ring-violet-200 outline-none text-sm">
                ...
                <div class="rounded-[1.75rem] bg-white/90 backdrop-blur-md border border-white/70 shadow-lg shadow-violet-200/40 p-6">
                    ...
                    <div class="space-y-3 mb-4 max-h-48 overflow-y-auto">
                        @forelse($chapter->comments->take(12) as $c)
                            <div class="flex gap-3">
                                <div class="w-8 h-8 rounded-full bg-violet-200 flex-shrink-0 flex items-center justify-center text-xs font-semibold text-violet-800">
                                    {{ strtoupper(substr($c->user->name ?? '?', 0, 1)) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="text-xs font-medium text-slate-800">
                                        {{ $c->user->name ?? 'Reader' }}
                                        <span class="text-slate-400 font-normal">· {{ $c->created_at?->diffForHumans() ?? '' }}</span>
                                    </p>
                                    @if($c->reaction)
                                        <p class="text-sm text-slate-600">{{ $c->reaction === 'heart' ? '❤️' : ($c->reaction === 'fire' ? '🔥' : '😢') }}</p>
                                    @endif
                                    @if($c->body)
                                        <p class="text-sm text-slate-600 break-words">{{ Str::limit($c->body, 200) }}</p>
                                    @endif
                                </div>
                            </div>
                        @empty
                ...
                <div class="rounded-[1.75rem] bg-white/90 backdrop-blur-md border border-white/70 shadow-lg shadow-violet-200/40 p-6">
                    ...
                    <p class="text-xs text-slate-500 mb-3">Editor canvas theme (reading theme is set when creating the story).</p>
...
<script>
function chapterEditorPageEdit() {
    return {
        quill: null,
        chapterId: {{ $chapter->id }},
        chapterTitle: @json($chapter->chapter_title),
        chapterNumber: {{ (int) $chapter->chapter_number }},
        status: @json($chapter->status),
        continueAfter: @json($chapter->continue_reading_after ? (string) (int) $chapter->continue_reading_after : ''),
        spotifyPlaylist: @json($chapter->spotify_playlist ?? ''),
        contentHtml: '',
        wordCount: {{ max(0, (int) ($chapter->word_count ?? 0)) }},
        readingTime: {{ max(1, (int) ($chapter->reading_time ?? $chapter->read_time ?? 1)) }},
        savedAt: @json($chapter->updated_at?->format('H:i:s') ?? ''),
        fullscreen: false,
        previewMode: false,
        youtubeUrl: @json($chapter->youtube_url ?? ''),
        audioPreview: @json($chapter->audio_file ?? ''),
        selectionComment: '',
        selectedRange: null,
        editorTheme: 'light',
        ...
        init() {
            ...
            const existingHtml = @json($chapter->content);
            if (existingHtml && String(existingHtml).trim()) {
                this.quill.setText('');
                this.quill.clipboard.dangerouslyPasteHTML(0, existingHtml);
            }
        ...
        async saveNow(finalSave = false) {
            ...
            const yt = (this.youtubeUrl && String(this.youtubeUrl).trim()) ? String(this.youtubeUrl).trim() : null;
            const payload = {
                ...
                youtube_url: yt,
                spotify_playlist: (this.spotifyPlaylist && String(this.spotifyPlaylist).trim()) ? String(this.spotifyPlaylist).trim() : null,
            };
